...
export default function DeckPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 pt-32 pb-20 md:px-8">
      <div className="mb-16">
        <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-[#9B8B6E]">The Deck</p>
        <h1 className="mt-4 font-serif text-[44px] leading-[1.05] text-[#161310]">{cards.length} cards</h1>
        <p className="mt-4 max-w-xl text-[15px] leading-[1.75] text-black/55">A symbolic system drawn from psychoanalytic thinking. Not for prediction. For reflection.</p>
      </div>
        ...
        <div key={category} className="mb-14">
          ...
          <div className="grid grid-cols-2 gap-px border border-black/8 bg-black/8 sm:grid-cols-3 md:grid-cols-4">
            {cards.filter((card) => card.category === category).map((card) => (
              <Link key={card.id} href={'/deck/' + card.slug} className="group bg-[#F4EFE4] p-4 transition-colors duration-300 hover:bg-[#161310] md:p-5">
                <PlaceholderThumb />
